...
export async function POST(req: NextRequest) {
  ...
  if (action === "create") {
    const id = randomId();
    return NextResponse.json(await createJohari(id, body.name));
  }

  if (action === "join") {
    const s = await mutateJohari(body.id, sess => {
  ...
  if (action === "start-setup") {
    const s = await mutateJohari(body.id, sess => { sess.phase = "setup"; });
    return s ? NextResponse.json(s) : NextResponse.json({ error: "not found" }, { status: 404 });
  }
